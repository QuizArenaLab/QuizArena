...
import { Crown, Key, Plus } from "lucide-react";
...
export async function PrimaryMarketplace({ userId }: { userId: string }) {
...
export function TertiaryMarketplace() {
  return (
    <>
      ...
      <section>
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Advanced events with exclusive rewards and rankings.
          </p>
        </div>
        <PremiumCompetitions />
      </section>
...
export async function SecondaryMarketplace({ userId }: { userId: string }) {
      ...
      <section className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="bg-navy p-6 text-center relative overflow-hidden">
            ...
            <div className="text-xs font-bold text-amber-400/80 uppercase tracking-widest mb-1 relative z-10">
              National Rank
            </div>
            <div className="text-4xl font-black text-white relative z-10">
              {hasRank ? `#${performance?.rank}` : "—"}
            </div>
          </div>
          <div className="p-5 space-y-4">
            <div className="flex justify-between items-center pb-3 border-b border-gray-100">
              ...
              <span className="text-sm font-black text-gray-900">
                {hasRank && performance?.rank != null
                  ? `#${Math.max(1, Math.floor(performance.rank / 3))}`
                  : "—"}
              </span>
            </div>
            <div className="flex justify-between items-center pb-3 border-b border-gray-100">
              ...
              <span className="text-sm font-black text-emerald-600">
                {hasRank && performance
                  ? `${Math.round(performance.averageAccuracy || 0)}%ile`
                  : "—"}
              </span>
            </div>
            <div className="flex justify-between items-center pb-3 border-b border-gray-100">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-black text-gray-900">
                {hasRank && performance?.rank != null
                  ? `#${Math.max(1, performance.rank - 12)}`
                  : "—"}
              </span>
            </div>
          </div>
