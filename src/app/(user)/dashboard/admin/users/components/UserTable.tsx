...
export function UserTable({ users, page, totalPages, total, limit, onPageChange, onInspect, isPending }: UserTableProps) {
  ...
  const getHealthBadge = (health: string) => {
    switch (health) {
      case "Healthy":
        return <span className="px-2 py-1 text-xs rounded-full bg-green-50 text-green-700">Healthy</span>;
      case "Warning":
        return <span className="px-2 py-1 text-xs rounded-full bg-amber-50 text-amber-700">Warning</span>;
      case "Restricted":
        return <span className="px-2 py-1 text-xs rounded-full bg-orange-50 text-orange-700">Restricted</span>;
      case "Suspended":
        return <span className="px-2 py-1 text-xs rounded-full bg-red-50 text-red-700">Suspended</span>;
      default:
        return <span className="px-2 py-1 text-xs rounded-full bg-gray-50 text-gray-600">{health}</span>;
    }
  };
  ...
  const getSubscriptionBadge = (sub: string) => {
    switch (sub) {
      case "Premium":
        return <span className="px-2 py-1 text-xs rounded-full bg-blue-50 text-blue-700">Premium</span>;
      case "Challenge Pass":
        return <span className="px-2 py-1 text-xs rounded-full bg-amber-50 text-amber-700">Challenge Pass</span>;
      default:
        return <span className="px-2 py-1 text-xs rounded-full bg-gray-50 text-gray-600">Free</span>;
    }
  };
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case "SUPER_ADMIN":
        return <span className="text-xs px-2 py-1 rounded bg-purple-50 text-purple-700">SUPER_ADMIN</span>;
      case "ADMIN":
        return <span className="text-xs px-2 py-1 rounded bg-red-50 text-red-700">ADMIN</span>;
      case "MODERATOR":
        return <span className="text-xs px-2 py-1 rounded bg-blue-50 text-blue-700">MODERATOR</span>;
      default:
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-100 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">User Health</th>
            ...
            {isPending && users.length === 0 ? (
              ...
              users.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          @{user.username} • {user.examCategory || "Unspecified"} • {user.preparationLevel ? user.preparationLevel.charAt(0) + user.preparationLevel.slice(1).toLowerCase() : "Beginner"}
                        </p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-sm text-gray-500">{formatTimeAgo(user.lastActiveAt)}</span>
                  </td>
                  <td className="px-4 py-3">
              ...
              <tr>
                <td colSpan={8} className="px-4 py-16">
                  <div className="flex flex-col items-center max-w-sm mx-auto text-center">
                    ...
                    <h3 className="text-sm font-semibold text-gray-900 mb-1">No matching users found.</h3>
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between shrink-0 bg-white z-10 relative">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2 rounded-lg border border-gray-200 ${
                page >= totalPages || isPending ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-50"
              }`}
            >
