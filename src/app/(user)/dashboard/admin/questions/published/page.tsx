...
import { ChevronLeft, ChevronRight, Eye, CheckCircle2, Archive } from "lucide-react";

import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Published Questions | Question Bank",
  description: "Browse all approved and published questions",
};
...
async function PublishedContent({
  ...
  const currentFilters = { search, category, subject, difficulty, status: "APPROVED" };
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        {result.questions.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-500">No published questions yet</p>
          </div>
        ) : (
          <>
                  ...
                  <tr key={q.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center gap-1.5 justify-end">
                        <Link
                          href={`/dashboard/admin/question-bank/${q.id}`}
...
export default function PublishedPage({
  ...
  return (
    ...
    >
      <PublishedContent searchParams={searchParams} />
    </Suspense>
  );
}
