...
export function ReportInvestigationDrawer({
  ...
  // Forms states
  const [activeForm, setActiveForm] = useState<"DISMISS" | "WARN" | "RESTRICT" | "SUSPEND" | null>(
    null
  );
  ...
  async function handleRestrict() {
    ...
    await performAction(() =>
      restrictUser(report.targetUser!.id, report.id, parseInt(restrictDuration), restrictReason)
    );
  }
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-[600px] bg-gray-50 z-50 shadow-2xl flex flex-col border-l border-gray-200 animate-in slide-in-from-right duration-300">
        ...
        <div className="flex items-center justify-between p-4 bg-white border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold text-[#0A1C40]">
              Investigation: {report.id.slice(0, 8).toUpperCase()}
            </h2>
          </div>
          <span
            className={`inline-flex px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${getPriorityClasses(report.priority)}`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 truncate">
                    {report.reportedBy.email || "No email provided"}
                  </p>
                </div>
              </div>
            </section>
              ...
              <section className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <span
                      className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${report.targetUser.accountState === "ACTIVE" ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
                    >
                ...
                {report.notes.map((note) => (
                  <div key={note.id} className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    ...
                    <span className="text-[10px] text-gray-400 mt-1 block">
                      By {note.author.name} &bull; {formatDate(note.createdAt)}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <section className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-4 bg-gray-50/50">
              {actionMessage && (
                <div
                  className={`p-3 rounded-lg text-xs font-semibold ${actionMessage.includes("failed") || actionMessage.includes("required") ? "bg-red-50 text-red-600 border border-red-200" : "bg-emerald-50 text-emerald-600 border border-emerald-200"}`}
                >
                  {actionMessage}
                </div>
              )}
              ...
              {!activeForm && (
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors flex flex-col items-center gap-2 shadow-sm"
                  >
                  ...
                  {report.targetUser && (
                    <>
                      <button
                        onClick={() => setActiveForm("WARN")}
                        className="px-4 py-3 bg-white border border-amber-200 rounded-xl text-sm font-semibold text-amber-700 hover:bg-amber-50 transition-colors flex flex-col items-center gap-2 shadow-sm"
                      >
                        <ShieldAlert className="w-5 h-5 text-amber-500" />
                        Warn User
                      </button>
                      <button
                        onClick={() => setActiveForm("RESTRICT")}
                        className="px-4 py-3 bg-white border border-orange-200 rounded-xl text-sm font-semibold text-orange-700 hover:bg-orange-50 transition-colors flex flex-col items-center gap-2 shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-3 bg-white border border-red-200 rounded-xl text-sm font-semibold text-red-700 hover:bg-red-50 transition-colors flex flex-col items-center gap-2 shadow-sm"
                      >
              ...
              {activeForm === "DISMISS" && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <select
                    ...
                    className="w-full text-sm p-3 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-[#0A1C40]"
                  >
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-gray-800 text-white text-sm font-semibold rounded-lg hover:bg-gray-900 transition-colors disabled:opacity-50"
                  >
                    Confirm Dismissal
                  </button>
                </div>
              ...
              {activeForm === "WARN" && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <select
                    ...
                    className="w-full text-sm p-3 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-amber-500"
                  >
                  ...
                  <button
                    onClick={handleWarn}
                    disabled={isActioning}
                    className="w-full py-2.5 bg-amber-500 text-white text-sm font-semibold rounded-lg hover:bg-amber-600 transition-colors disabled:opacity-50"
                  >
                    Send Warning
                  </button>
                </div>
              ...
              {activeForm === "RESTRICT" && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        className="w-full text-sm p-2.5 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-orange-500"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="E.g., Repeated cheating..."
                        className="w-full text-sm p-2.5 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-orange-500"
                      />
                    </div>
                  </div>
                  <button
                    ...
                    disabled={isActioning}
                    className="w-full py-2.5 bg-orange-600 text-white text-sm font-semibold rounded-lg hover:bg-orange-700 transition-colors disabled:opacity-50"
                  >
                    Apply Restriction
                  </button>
                </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 border-l-4 border-red-500 pl-4 py-2">
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-xs text-gray-600 font-medium">
                      I understand this action permanently removes the user&apos;s access to
                      QuizArena.
                    </span>
                  </label>
