    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-md w-[500px] overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>{issue?.issueFingerprint}</SheetDescription>
        </SheetHeader>
          ...
          <div className="mt-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm border rounded-md p-3 bg-white">
                ...
                <div>
                  <span className="text-muted-foreground">Previous Severity:</span>
                  <br />
            ...
            <div className="space-y-3 border-l-4 border-primary pl-4 py-1">
              ...
              <p className="text-sm text-muted-foreground">
                {issue.issueType === "MISSING_EXPLANATION" &&
                  "The question is missing an explanation. Add a detailed explanation to help users understand the correct answer."}
                {issue.issueType === "DUPLICATE_CANDIDATE" &&
                  "Merge this question with the detected candidates or archive it if it is redundant."}
                {issue.issueType === "HIGH_REPORTS" &&
                  "Inspect the reports and evaluate if the question is factually incorrect or ambiguous."}
                {issue.issueType === "LOW_CONFIDENCE" &&
                  "Data quality is LOW. Wait for more attempts or adjust the question difficulty."}
                {issue.issueType === "UNUSED" &&
                  "Include this question in future practice pools or tournaments."}
                {issue.issueType === "OUTDATED" &&
                  "Review this dynamic question to ensure its content is still relevant."}
                {issue.issueType === "NO_ATTEMPTS" &&
                  "This question was served but not attempted. It might be too long or skipped often."}
              </p>
            </div>
                    ...
                    <div key={audit.id} className="text-xs border p-2 rounded bg-slate-50">
                      <div className="flex justify-between font-medium">
                        ...
                        <span className="text-muted-foreground">
                          {format(new Date(audit.createdAt), "PP")}
                        </span>
                      </div>
                      {audit.actor && (
                        <div className="text-muted-foreground mt-1">By: {audit.actor.name}</div>
                      )}
                      {audit.reason && (
        ...
        <SheetFooter className="mt-6 flex sm:justify-between border-t pt-4">
          <Button
            variant="outline"
            onClick={() =>
              (window.location.href = `/dashboard/admin/question-bank/${issue?.questionId}/edit`)
            }
          >
            Edit Question
          </Button>
