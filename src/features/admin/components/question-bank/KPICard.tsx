  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <p
          className={`font-bold text-slate-500 uppercase tracking-widest ${isPrimary ? "text-xs" : "text-[11px]"}`}
        >
          {label}
        </p>
      ...
      <div>
        <p
          className={`font-extrabold tracking-tight text-slate-900 tabular-nums ${isPrimary ? "text-4xl" : "text-2xl"}`}
        >
        ...
        {(subtitle || trend) && (
          <div className="mt-3 flex items-center gap-2">
            {trend && (
              <span
                className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full ${trend.positive ? "bg-emerald-50 text-emerald-700 border border-emerald-200/50" : "bg-rose-50 text-rose-700 border border-rose-200/50"}`}
              >
                {trend.positive ? "+" : "-"}
                {trend.value}%
              </span>
