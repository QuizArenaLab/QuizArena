...
const yearPickerManifest: DateManifest = {
  id: "core-yearpicker",
  name: "YearPicker",
  category: "date",
  version: "1.0.0",
...
export function YearPicker({ name, disabled, required }: YearPickerProps) {
  return (
    <BaseField
      name={name}
      controlled={true}
      manifest={yearPickerManifest}
      disabled={disabled}
      required={required}
    >
      {(context) => (
        <div className="flex flex-col space-y-2">
          ...
          >
            <span className="text-sm flex-1 text-gray-700">Select Year</span>
            <span>📅</span>
          </div>
