...
const monthPickerManifest: DateManifest = {
  id: "core-monthpicker",
  name: "MonthPicker",
  category: "date",
  version: "1.0.0",
...
export function MonthPicker({ name, disabled, required }: MonthPickerProps) {
  return (
    <BaseField
      name={name}
      controlled={true}
      manifest={monthPickerManifest}
      disabled={disabled}
      required={required}
    >
      {(context) => (
        <div className="flex flex-col space-y-2">
          ...
          >
            <span className="text-sm flex-1 text-gray-700">Select Month</span>
            <span>📅</span>
          </div>
