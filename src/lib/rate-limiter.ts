...
class RateLimiter {
  ...
  check(key: string): { isLimited: boolean; remaining: number; resetAt: number | null } {
    ...
    // Persist filtered attempts back to map
  ...
  /**
   * Atomic check and register operation to prevent TOCTOU races
   */
  checkAndRegister(key: string): { isLimited: boolean; remaining: number; resetAt: number | null } {
    const now = Date.now();
    const attempts = this.attempts.get(key) ?? [];
    const validAttempts = attempts.filter((timestamp) => now - timestamp < this.windowMs);
    ...
    if (validAttempts.length >= this.maxAttempts) {
      const oldest = validAttempts[0];
      const resetAt = oldest + this.windowMs;
      this.attempts.set(key, validAttempts); // Persist cleanup
      return { isLimited: true, remaining: 0, resetAt };
    }
...
/**
 * Starts the periodic cleanup of rate limiter attempts.
